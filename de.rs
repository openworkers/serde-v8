...
impl<'de> de::Deserializer<'de> for &'_ mut Deserializer<'_, '_, '_> {
        ...
        V: Visitor<'de>,
    {
        match name {
            ...
            BigInt::MAGIC_NAME => visit_magic(visitor, BigInt::from_v8(self.scope, self.input)?),
        ...
        V: Visitor<'de>,
    {
        ...
        // Struct or tuple variant
            ...
            let tag = {
                ...
                let prop_names =
                    prop_names.ok_or_else(|| Error::ExpectedEnum(self.input.type_repr()))?;
...
impl<'a, 's, 'i> MapObjectAccess<'a, 's, 'i> {
    pub fn new(obj: v8::Local<'a, v8::Object>, scope: &'a mut v8::PinScope<'s, 'i>) -> Self {
...
fn to_utf8_fast(s: v8::Local<v8::String>, scope: &mut v8::PinScope) -> Option<String> {
