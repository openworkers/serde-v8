    ...
    T: V8Sliceable,
{
    ...
    /// buffers).
    pub unsafe fn from_parts(store: v8::SharedRef<v8::BackingStore>, range: Range<usize>) -> Self {
        Self {
            store,
            range: range.start / std::mem::size_of::<T>()..range.end / std::mem::size_of::<T>(),
    ...
    pub fn into_parts(self) -> (v8::SharedRef<v8::BackingStore>, Range<usize>) {
        (
            self.store,
            self.range.start * std::mem::size_of::<T>()..self.range.end * std::mem::size_of::<T>(),
        )
    }
...
mod tests {
    ...
    fn make_slice<T: V8Sliceable>(len: usize) -> V8Slice<T> {
        let slice = V8Slice::<T>::very_unsafe_new_only_for_test(len * std::mem::size_of::<T>());
